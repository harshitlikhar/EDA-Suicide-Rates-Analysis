...
For the purpose of completing our final project, we have decided to choose the World Bank Dataset. This dataset contains information about the suicides happening across the world from 1985-2015. Evaluating this 30 years data with respect to countries, sex, age, GDP was really insightful and led to many interesting conclusions. 

However, a lot of our conclusions were limited due to lack of data. Quite a few major countries including China, India, etc were missing, thus our results could not include the suicide rates of the two most populated countries of the world.

Let's see how we are able to gain insights from this data!

### Data Description:

This dataset consists of 12 columns and about 27000 rows ranging from 1985-2015 for each country. Columns and their information is mentioned below.

```{r pressure, echo=FALSE, fig.align = 'center', fig.cap="Data Info", out.width = '35%'}
knitr::include_graphics("temp.png")
```

Thus, in order to conduct a statistical analysis, our first task was to perform data cleaning.

#### Data Cleaning

* We removed 7 countries from our dataset which had less than 3 years of data thus accounting for less than 10% of data per country.
* Human Development Index(HDI) column consisted of more than 66% of the missing values, thus we dropped that column from our analysis.
* We made a new column : continent, which was derived from the countrycode package. This would help us evaluate the suicide rates across the continents with respect to various years.
* We dropped one more column named country-year because it was giving no additional information and just a combination of year and country.
...
```{r}
## note: there appears to be an issue with 2016 data. not only are there few countries with data, but those that do have data are incomplete
...
We begin our analysis by examining the suicide rate per 100000 people  and the life expectancy in the year 2007, the
most recent year. This will help us understand if there is any trend now and then we can compare it to historical data
available to us.
...
plot1 <- data %>%
  ...
  geom_line(col = "red") + 
  geom_point(col = "red") + 
  labs(title = "Suicide Trend around the Globe (per 100k)",
       x = "Year", 
       y = "No. of suicides per 100k") + 
  scale_x_continuous(breaks = seq(1985, 2015,3)) + theme_bw() + theme(plot.title = element_text(face="bold.italic", size=14, family = "Alegreya", hjust = .5))
```
...
plot2 <- data %>%
  ...
  labs(title = "Average GDP per capita around the Globe",
       x = "Year", 
       y = "Average GDP per capita") + 
  scale_x_continuous(breaks = seq(1985, 2015,3)) + theme_bw() +
...
continent_time_plot <- ggplot(continent_time, aes(x = year, y = suicide_per_100k, color = continent)) + 
  geom_line(size=0.85) + 
  labs(title = "Trends Over Time, by Continent", 
       x = "Year", 
       y = "Suicides per 100k", 
       color = "Continent") + scale_color_hue(l=78, c=78) + theme_bw() +
...
grid.arrange(continent_time_plot, nrow = 1)
```
## Insights
...
gender_time_plot <- data %>%
  group_by(year, sex) %>%
  summarize(suicide_per_100k = (sum(as.numeric(suicides_no)) / sum(as.numeric(population))) * 100000) %>%
  ggplot(aes(x = year, y = suicide_per_100k, color = sex)) + 
  geom_line(size=0.85) + geom_point() +
  labs(title = "Trends Over Time, by Sex", 
       x = "Year", 
       y = "Suicides per 100k", 
       color = "Sex") + theme_bw() +
  theme(plot.title = element_text(face="bold.italic", size = 12, family="Alegreya", hjust=.5)) +
  scale_x_continuous(breaks = seq(1985, 2015, 2), minor_breaks = F) + scale_color_manual(values = c("Female" = "#ff69b4", "Male" = "#00bfff"))

gender_plot_hue <- data %>%
  ...
  geom_bar(stat = "identity", position = "dodge") + theme_bw() +  theme(plot.title = element_text(face="bold.italic", size = 10, family="Alegreya", hjust=.5)) + 
 
  labs(title = "Age Disparity, by Continent",
       x = "Continent", 
       y = "Suicides per 100k", 
       fill = "sex") + scale_fill_manual(values = c("Female" = "#ff69b4", "Male" = "#00bfff"))

grid.arrange( gender_time_plot,gender_plot_hue, ncol = 2)
```
...
data %>%
  ...
  labs(title = "Age Disparity, by Continent",
       x = "Continent", 
       y = "Suicides per 100k", 
       fill = "Age") + scale_fill_manual(values = c("5-14" = "#1804fb", "15-24" = "#7e51eb", "25-34" = "#44c8de", "35-54" = "#c66be1", "55-74" = "#f6c63e", "75+" = "#f19380")) +  theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) 
```
...
ggplot(country_mean_gdp, aes(x = gdp_per_capita, y = suicide_per_100k, col = continent)) + 
  geom_point() +  theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) +
...
```{r}
ggplot(gdp_suicide_no_outliers, aes(x = gdp_per_capita, y = suicide_per_100k, color = continent)) + 
  geom_point() + 
  geom_smooth(method = "lm", aes(group = 1)) + 
  scale_x_continuous(labels=scales::dollar_format(prefix="$"), breaks = seq(0, 70000, 10000)) +  theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) +
  labs(title = "Correlation between GDP (per capita) and Suicides per 100k", 
       x = "GDP (per capita)", 
       y = "Suicides per 100k", 
       col = "Continent") + 
...
data %>%
  ...
  scale_x_continuous(breaks = seq(1985, 2015, 5), minor_breaks = NULL) +  theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) +
...
generation_rate <- data %>%
  group_by(generation,year,sex) %>%
  summarize(suicide_per_100k = (sum(as.numeric(suicides_no)) / sum(as.numeric(population))) * 100000) %>%
  ggplot(aes(x = year, y = suicide_per_100k, color = sex)) + 
  geom_point() + 
  geom_line() + 
  facet_wrap(generation ~ ., scales = "free_y") +  theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) +
  scale_x_continuous(breaks = seq(1985, 2015, 5), minor_breaks = NULL) +
  labs(title = "Suicides per 100k by Gender faceted by Generation", 
       x = "Year", 
       y = "Suicides per 100k") 
...
```{r}
cbbPalette <- c( "#1804fb", "#7e51eb", "#44c8de", "#c66be1", "#f6c63e","#f19380")
generation_rate1 <- data %>%
  group_by(generation) %>%
  summarize(suicide_per_100k = (sum(as.numeric(suicides_no)) / sum(as.numeric(population))) * 100000) %>%
  ggplot(aes(x=generation, y =suicide_per_100k, fill=generation)) + 
  geom_bar(stat = "identity", position = "dodge") + scale_fill_manual(values=cbbPalette) + theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) + labs(title = "Suicides per 100k, by Generation", 
       x = "Generation", 
       y = "Suicides per 100k") 
generation_rate1
```
...
data_filtered %>%
  ...
  geom_line() + 
  geom_hline(yintercept = global_average, alpha=0.7) + 
  scale_x_continuous(breaks = seq(1985, 2015, 5), minor_breaks = F) + theme_bw() + theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) + 
  labs(title = "Suicides per 100k population, 1985 - 2015", 
       subtitle = "for countries : United States, Russian Federation, Australia, Mauritius, Republic of Korea, Lithuania", 
       x = "Year", 
       y = "Suicides per 100k", 
       col = "Country")
...
data_filtered %>%
  ...
  facet_wrap(~ sex, ncol = 2) + theme_bw() + theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) +
  labs(title = "Suicides per 100k population, 1985 - 2015 by gender", 
       subtitle = "for countries : United States, Russian Federation, Australia, Mauritius, Republic of Korea, Lithuania", 
       x = "Year", 
       y = "Suicides per 100k", 
       col = "Country")
...
generation_rate1 <- data %>%
  ...
  summarize(suicide_per_100k = (sum(as.numeric(suicides_no)) / sum(as.numeric(population))) * 100000) %>%
  ggplot(aes(x=year, y =suicide_per_100k, color=continent)) + geom_line(aes(group=country),size=0.261,alpha=0.4) + geom_smooth(method='gam',lwd=1, se=FALSE) + facet_wrap(~continent, nrow = 2)+ theme_bw()+theme(plot.title = element_text(face="bold.italic", size=10, family = "Alegreya", hjust = .5)) + labs(title = "Suicides per 100k, by Continent", 
       x = "Year", 
       y = "Suicides per 100k") 
generation_rate1
# 
# ggplot(data,aes(y=lifeExp, x=year,color=continent)) + geom_line(aes(group=country),size=0.261,alpha=0.4) + geom_smooth(method='gam',lwd=1.5) + facet_wrap(~continent, nrow = 1) + labs(title="Figure 5: Plot between Life Expectancy of each country and Year",
...
<!-- ## 3.4.2 Men - Ages 15-24, 25-34 & 35-54 -->
...
<!-- * If you’ve enjoyed my analysis, check out my recent XGBoost kernel here and give me a follow to see future work! -->
